...
import space.beka.yolharakatqoidalari.db.MyDbHelper.Companion.LIKE_OGOH as LIKE_OGOH1
...
class MyDbHelper(context: Context?)
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val queryOgogh = "create table $TABLE_NAME_OGOH ($ID_OGOH integer not null primary key autoincrement unique, $NAME_OGOH text not null, $MATN_OGOH text not null, $IMAGE_OGOH text not null, $LIKE_OGOH1 integer not null, $CATEGORY integer not null)"
    ...
    override fun addLabel(belgi: Rule) {
    ...
    override fun deleteLabel(belgi: Rule) {
        val database = this.writableDatabase
        database.delete(TABLE_NAME_OGOH, "$ID_OGOH = ?", arrayOf("${belgi.id}"))
        database.close()
    }

    override fun getAllLabel(): ArrayList<Rule> {
        ...
        if (cursor.moveToFirst()){
            do{
                ...
                val contact = Rule(id, name, matni, imagePath, like, category)
                list.add(contact)
